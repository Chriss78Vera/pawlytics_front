...
export function AdminMascotasPage({ onHistory }) {
  ...
  const loadMascotas = async (nextPage = page) => {
    ...
    try {
      const response = await pawlyticsApi.getMascotas({ page: nextPage, limit: 5, filters });
...
function MascotasFilters({ filters, onChange, onSubmit }) {
  ...
  return (
    <div className="grid gap-3 rounded-2xl bg-white p-4 shadow border border-[#7EE081]/20 md:grid-cols-6">
      ...
      <SearchableSelect
        ...
        clearable
      />
      <FilterInput label="Propietario" value={filters.owner} onChange={(value) => onChange('owner', value)} />
